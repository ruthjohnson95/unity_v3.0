...
do
	...
	# get naming prefix 
	PREFIX=$(basename $LD_FILE | cut -d'.' -f1,2,3)
	...
	python scripts/simulate.py --sim_name $SIM_NAME --h_sim $H_SIM --p_sim $P_SIM --N $N --blocks $BLOCKS --seed $SEED --ld_dir $LD_DIR --outdir $OUTDIR
        ...
        # transform betas 
	python scripts/transform_betas.py --gwas_dir $OUTDIR --ld_file $LD_FILE
	...
	python scripts/half_ld.py --ld_file $LD_FILE --ld_half_dir $LD_HALF_DIR 
        ...
        # run inference 
	LD_HALF_FILE=$LD_HALF_DIR/$PREFIX.ld_half
	GWAS_FILE=$OUTDIR/$PREFIX.gwas 
	python src/unity_v3.py --seed $SEED --H_gwas  $H_GWAS --H_snp $H_SNP --N $N --id $SIM_NAME --ld_half_file  $LD_HALF_FILE --gwas_file $GWAS_FILE --outdir $OUTDIR --its $ITS
